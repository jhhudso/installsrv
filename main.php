...
function additem(printbarcode) {
	...
	if (namebox.value.length < 1 || referencebox.value.length < 1 || pricesellbox.value.length < 1 || isNaN(Number(pricesellbox.value))) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = (function(x,name,pricesell,printbarcode) {		
    	return function() {
    		     	...
    		     	if (response["result"] === true) {
    					...
    					document.getElementById("addform").reset();
      					...
      					if (printbarcode) {
        					...
        					barcodexhttp.open("GET", "barcode.php?item_name="+name+"&item_code="+response["code"]+"&item_pricesell="+pricesell+"&print=1", true);
        					barcodexhttp.send();
      					}	
    		     	} else {           		     	
	...
	})(xhttp,namebox.value,pricesellbox.value,printbarcode)
		  
	xhttp.open("GET", "additem.php?name="+namebox.value+"&reference="+referencebox.value+"&code=0&pricesell="+pricesellbox.value+"&category="+categoryselect.value, true);	  
	xhttp.send();	
}
